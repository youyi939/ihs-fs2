...
public interface HeatNetworkDataMapper extends BaseMapper<HeatNetworkData> {
     ...
     * @return
     */
    @Select("SELECT * FROM heat_network_data WHERE gmt_create in (SELECT max(gmt_create) FROM heat_network_data GROUP BY station_id);")
    List<HeatNetworkData> selectDataList();
